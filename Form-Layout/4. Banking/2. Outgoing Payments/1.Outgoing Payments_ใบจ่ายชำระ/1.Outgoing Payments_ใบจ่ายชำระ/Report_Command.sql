...
SELECT 
    ...
    OcrCode2 = CASE 
        WHEN T2.InvType = 18 THEN (SELECT TOP 1 OcrCode2 FROM PCH1 WHERE DocEntry = T2.DocEntry AND ISNULL(OcrCode2, '') <> '')
        WHEN T2.InvType = 204 THEN (SELECT TOP 1 OcrCode2 FROM DPO1 WHERE DocEntry = T2.DocEntry AND ISNULL(OcrCode2, '') <> '')
        WHEN T2.InvType = 19 THEN (SELECT TOP 1 OcrCode2 FROM RPC1 WHERE DocEntry = T2.DocEntry AND ISNULL(OcrCode2, '') <> '')
        WHEN T2.InvType IN (46, 30) THEN (SELECT TOP 1 OcrCode2 FROM JDT1 WHERE TransId = T2.DocTransId AND ISNULL(OcrCode2, '') <> '')
        ELSE NULL
    END,

    OcrName = (
        SELECT TOP 1 OcrName FROM OOCR WHERE OcrCode = CASE 
            WHEN T2.InvType = 18 THEN (SELECT TOP 1 OcrCode2 FROM PCH1 WHERE DocEntry = T2.DocEntry AND ISNULL(OcrCode2, '') <> '')
            WHEN T2.InvType = 204 THEN (SELECT TOP 1 OcrCode2 FROM DPO1 WHERE DocEntry = T2.DocEntry AND ISNULL(OcrCode2, '') <> '')
            WHEN T2.InvType = 19 THEN (SELECT TOP 1 OcrCode2 FROM RPC1 WHERE DocEntry = T2.DocEntry AND ISNULL(OcrCode2, '') <> '')
            WHEN T2.InvType IN (46, 30) THEN (SELECT TOP 1 OcrCode2 FROM JDT1 WHERE TransId = T2.DocTransId AND ISNULL(OcrCode2, '') <> '')
